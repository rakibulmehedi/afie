"""Pipeline orchestrator for the CSPE synthesis saga.

Called as a FastAPI background task after ingest_to_db has committed the
ingest_queue row.  Drives the saga from RECEIVED through SYNTHESIZING,
DRAFTED, and finally AWAITING_APPROVAL — or to FAILED if all LLM retries
are exhausted.

Errors are logged but NOT re-raised; the caller has already returned HTTP 200
to QStash.
"""
...
async def orchestrate_pipeline(payload: ConsumePayload) -> None:
    """Full synthesis pipeline: ingest_queue → RECEIVED saga → SYNTHESIZING → DRAFTED → AWAITING_APPROVAL.

    Called as a background task AFTER ingest_to_db has inserted the ingest_queue row.

    Errors are logged but NOT re-raised (caller already returned HTTP 200 to QStash).
    """
    try:
        ...
        if pool is None:
            logger.error(
                "orchestrate_pipeline: pool not initialised — tenant=%s",
                payload.tenant_id,
            )
            return
        ...
        async with pool.connection() as conn:
            async with conn.transaction():
                ...
                saga_id = await create_saga(conn, ingest_id, payload.tenant_id)

                # 4. RECEIVED → SYNTHESIZING (must commit BEFORE calling LLM)
                advanced = await advance_saga(
                    conn,
                    saga_id,
                    payload.tenant_id,
                    SagaStatus.RECEIVED,
        ...
        # ------------------------------------------------------------------
        from app.synthesis.frameworks import get_synthesis_model  # noqa: PLC0415
        from app.synthesis.persona import build_system_prompt  # noqa: PLC0415
        from app.synthesis.router import (  # noqa: PLC0415
            SynthesisError,
        ...
        if llm_output is None:
            ...
            async with pool.connection() as conn2:
                async with conn2.transaction():
                    ...
                    from app.db.saga import advance_saga as _adv  # noqa: PLC0415
        ...
        async with pool.connection() as conn3:
            async with conn3.transaction():
                ...
                from app.db.saga import (  # noqa: PLC0415
                    advance_saga as _adv,
                    mark_ingest_processed as _mip,
                )
                ...
                await _mip(conn3, ingest_id, payload.tenant_id)
        ...
        logger.exception(
            "orchestrate_pipeline failed tenant=%s idem=%s",
            payload.tenant_id,
            payload.idempotency_key,
        )
